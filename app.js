...
function fetchSteamApi(endpoint) {
  return fetch(`https://store.steampowered.com${endpoint}`).then((response) => response.json());
}
...
function fetchHl2bApi(name) {
  ...
  return fetch(`https://howlongtobeat.com/api/search`, init).then((response) => response.json());
}

app.get('/:steamId',asyncHandler(async (req, res) => {
